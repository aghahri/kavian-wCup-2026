...
function buildCrowdRow(match: Match, predictions: { homeScore: number; awayScore: number }[]): CrowdMatchRow | null {
...
export async function getCrowdForMatch(match: Match): Promise<CrowdMatchRow | null> {
  const predictions = await prisma.prediction.findMany({
    where: { matchId: match.id },
    select: { homeScore: true, awayScore: true },
  });
  return buildCrowdRow(match, predictions);
}
...
export async function buildCrowdData(_locale: Locale): Promise<{
  ...
  for (const match of matches) {
    ...
    const row = buildCrowdRow(match, predictions);
